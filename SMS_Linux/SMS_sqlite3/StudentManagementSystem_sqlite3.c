...
void exit_system(Student *head)
{
	...
	while(p)
	{
		...
		sprintf(sql, "insert into student values (%d, %d, '%s', '%s');", p->id, p->age, p->name, p->tel);
		if(sqlite3_exec(ppdb, sql, NULL, NULL, NULL) != SQLITE_OK)
		{
			//continue;
		}
...
void readData(Student *head)
{
	...
	if(sqlite3_open("student.db", &ppdb) != SQLITE_OK)
	{
		//第一次打开会出错，因为在系统关闭的时候才创建数据库，所以忽略第一次打开的错误
		return;
	}
	...
	//从数据读取数据，设置回调函数createLink, 回调函数参数为head链表头指针
	if(sqlite3_exec(ppdb, sql, createLink, (void *)head, NULL) != SQLITE_OK)
	{
		printf("sqlite3_exec 2 error\n");
	}
}
